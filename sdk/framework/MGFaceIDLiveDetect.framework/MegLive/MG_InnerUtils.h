...
typedef struct {
    ...
    std::vector<unsigned char> (*compress_jpeg)(const unsigned char* img_data, size_t width, size_t height,
                                                int quality);
    std::string (*gen_result_file)(const std::vector<std::vector<unsigned char> >& inputs, std::vector<unsigned char> &buffer);
    std::string (*gen_result_file_with_phonefinger)(const std::vector<std::vector<unsigned char> >& inputs,
                                                    const std::vector<unsigned char>& phone_finger, std::vector<unsigned char> &buffer);
